...
void *eliminate(void *param)
{
    ...
    ustart = (unsigned long long)starttm.tv_sec * 1000000 + starttm.tv_usec;
    ustop = (unsigned long long)stoptm.tv_sec * 1000000 + stoptm.tv_usec;

    printf("===========THREAD %d. uses = %g ms.\n",i, (float)(ustop - ustart)/(float)1000);
