...
int main(int argc, char **argv) {
  ...
  for(norm = 0; norm < N; norm++)
  {
    ...
    for(i=0; i<procs; i++)
    {
      /* exchanging data it needs to perform the calculation */
      if(i != map[norm])
      {
        // printf("sending %d %d from %d to %d\n",norm,norm, myrank,i);
        MPI_Isend( &A[norm][norm], N-norm, MPI_FLOAT, i,0, MPI_COMM_WORLD,&request);
        MPI_Isend( &B[norm], 1, MPI_FLOAT, i,0, MPI_COMM_WORLD,&request);
      }
      else
      {
        // printf("%d receiving %d %d from %d\n",myrank, norm, norm, i);
        MPI_Recv(&A[norm][norm], N-norm, MPI_FLOAT, i,0, MPI_COMM_WORLD,&status);
        MPI_Recv(&B[norm], 1, MPI_FLOAT,i,0, MPI_COMM_WORLD,&status);
      }
